...
combine = c(scale(df2$Anxiety + df2$Depression + df2$Insomnia + df2$OCD))
# Converting Tables
df3 = df2 %>%
...
Another significant variable on the table is the **Std Combined MH**, also known as the Standardized Mental Health Score for each respondent. This specific variable was calculated by standardizing the four mental health-related variables (**Depression**, **Anxiety**, **Insomnia**, and **OCD**) and taking their sum. This score reflects the overall mental health status of each individual, considering the contributions of Depression, Anxiety, Insomnia, and OCD. The importance of standardization can be observed after viewing the histogram below, which represents the distribution of scores for each of the four mental health conditions: 
...
Group Three aims to investigate two questions: “What is the most effective model for assessing cumulative mental health scores?” and “What is the optimal model for evaluating each individual mental health condition?” Given that these two questions evaluate similar predictions, Group Three decided to utilize a similar modeling structure to predict all five variables and investigate the optimal model for predictions.

<br>

Due to the presence of binary variables and a small amount of observations, Group Three resorted to cross-validation instead of the traditional train and test approach. Cross-validation fits the original dataset into ten folds of training and testing datasets, and uses linear regression models as the modeling method. Group three analyzed the two questions by constructing and comparing five models, each individually applied to every observation in the data set. To determine the model with the best performance, group three condensed each model’s average Mean Absolute Error (MAE) into a table. MAE is a measure of how far, on average, the predictions of a model are from the actual combined mental health scores. 

<br>

Starting with all variables, Group Three gradually reduced the number of predictors to determine which ones are the most informative. Group Three then plotted the models together with an empty linear regression model. The empty model in this scenario functions as a reference model to assess each model's predicting ability. In the context of our data, the empty model predicts a person's mental health score without considering any variables related to their background and listening habits. Observe each model's MAE in the table below:

<br>

### Question 1: Standardized Mental Health Scores Prediction

<br>
<blockquote>

The plot below displays the five models that aim to predict the standardized mental health scores with various predictors. The x-axis on the plot is the variable **Hours Per Day**. 
...
# `All Variables`, `All Variables + Age * Country [Frequency]`, `Only Significant Variables`, `Only Significant Variables and Interactions`, `Empty`
...
Model = color_tile("white", "coral")(Model),
    ...
    MAE = color_tile("white", "#4b9173")(round(MAE,3))) %>%
...
It appears that the second model provides the best information to predict the mental health score more accurately. On the contrary, the lack of any variables in the empty model performs the worst in predicting mental health score as expected. Thus, we conclude that model that contains the combination of all individual predictors and the best interaction effect can predict the Standardized Mental Health Scores the best. However, none of the models fit the observations by simple visualization. We will now use similar modelling structures to examine each individual mental health conditions. 

<br>

### Q2: Individual Mental Health Scores Predictions

<br>

We will start by examining the mental health condition **Depression**. 
...
Next, we will look at the next mental health condition, **OCD**.
...
```
<br>
Given the table above, model one predicts the best score for **Anxiety**. Interestingly, this is also the only mental health condition that has model one as the best-performing model. 
...
<a href="https://www.kaggle.com/datasets/catherinerasgaitis/mxmh-survey-results" style="color: #A7C7E7;">Music & Mental Health Survey Results</a> dataset. This exploration was done through the investigation of two questions: "What is the most effective model for assessing cumulative mental health scores?" and "What is the optimal model for evaluating each individual mental health condition?" The group was driven to explore these questions to find valuable insights to the field of music therapy and mental health research. With question one, Group Three found that model two for the combined mental health scores, which had the lowest MAE, had the best model; while the empty model performed the worst. With question two, Group Three found that all the model evaluations have similar structures. It could be observed that the second model, which had all variables and one specific interaction, was the best for predicting the score for Depression, OCD, and Insomnia. However, the first model, which examined all variables alone, was the best for predicting the score for Anxiety. With this in mind, it is expected that Anxiety performs best as subjects are more likely to report higher levels of Anxiety as it doesn’t need a medical diagnosis. These findings lead to an ultimate conclusion that mental health conditions cannot be accurately predicted using music preferences alone, which the group did not expect. 

While aiming to identify effective models for assessing cumulative mental health scores and individual mental health conditions, Group Three faced challenges due to the fact the majority of variables are categorical variables with response 'yes' and 'no' in the dataset. Moreover, the number of unique combinations is significantly larger than the number of observations in our dataset, thus, the model may struggle to capture all the variations adequately, potentially leading to inaccuracies in predictions. Additionally, the use of categorical variables, such as the frequency categories "Never," "Rarely," "Sometimes," and "Very Frequently," introduces another source of potential error. Group Three’s binary conversion loses the inherent order and magnitude of these categories, potentially leading to a loss of information and affecting the model's ability to understand the true relationships between variables.

To mitigate these errors it would be integral to use different approaches. For example, utilizing the bootstrap method would help with the limited sample size and allow for more reliable parameter estimates. Additionally, implementing non-linear regression models such as polynomial regression to handle skewness in the mental health score distributions, or converting categorical variables into numerical representations using techniques like one-hot encoding to create binary features for each category would also be beneficial. This approach preserves the information of the categorical variables. 
...
In the real world, understanding the therapeutic potential of music remains crucial for improving mental health interventions. However, the findings underscore the importance of data availability and sample size in generating reliable and impactful results. Moving forward, Group Three encourages researchers to collect more extensive and diverse datasets (e.g. clinically reported mental health scores), as well as use the previously mentioned approaches to optimize estimates and models. By doing so, the true potential of music therapy and its role in promoting holistic well-being can be unlocked for individuals worldwide. 
